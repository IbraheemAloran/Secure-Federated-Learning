...
def detectHonest(score, nobyz, k):
  ...
  plt.legend(["honest", "not honest", "not honest", "not honest",], loc ="upper right")
  
  plt.grid()
  plt.title("Proposed Method")
  plt.show()
  real_label=np.zeros(100)
...
#Input: Takes in a list of malicious scores and the number of malicious clients
#Output: Returns the result and accuracy of the detection function
#Method: Clusters scores into 2 clusters and takes the higher average score as the malciious one. Then it calculates the accuracy of the detection
def detection(score, nobyz):
    estimator = KMeans(n_clusters=2)  #cluster scores into 2 clusters
    estimator.fit(score.reshape(-1, 1)) 
    label_pred = estimator.labels_  #get the list of labels from clusters
    y = np.ones(len(score[label_pred==0]))
    z = np.ones(len(score[label_pred==1]))
    print("LABELS: ", label_pred)
    print("SCORES: ", score)
    plt.plot(score[label_pred==0], y, 'o')
    plt.plot(score[label_pred==1], z, 'x')
    plt.show()
    print("LABELS: ", label_pred)
    if np.mean(score[label_pred==0])<np.mean(score[label_pred==1]): #find the cluster with the larger average suspicious score
    ...
    plt.plot(score[real_label==1], z, 'x')
    plt.show()
...
#Input: Takes in a list of malicious scores and the number of malicious clients
#Output: returns 1 if attack was detected and 0 otherwise
#Method: Uses Gap statistics to determine number of clusters and if an attack was detected
#def detection1(score, nobyz):
    # select_k = 0
    # nrefs = 10
    # ks = range(1, 8)
    ...
    # score = (score - min)/(max-min)
    # for i, k in enumerate(ks):
    #     estimator = KMeans(n_clusters=k) #initiate number of clusters from 1-8
    #     estimator.fit(score.reshape(-1, 1)) #cluster the malicious scores into k clusters
    #     label_pred = estimator.labels_ #list of labels of clustered clients
    #     center = estimator.cluster_centers_ #list of cluster centers/centroids

    #     #Perform Gap Statistics to find if scores can be clustered into more than 1 cluster
    #     Wk = np.sum([np.square(score[m]-center[label_pred[m]]) for m in range(len(score))]) 
    #     WkRef = np.zeros(nrefs)
    #     for j in range(nrefs):
    #         rand = np.random.uniform(0, 1, len(score))
    ...
    #     if i > 0:
    #         gapDiff[i - 1] = gaps[i - 1] - gaps[i] + sdk[i]
    # print("GAP DIFF: ", gapDiff)
    # for i in range(len(gapDiff)):
    #     if gapDiff[i] >= 0:
    #         select_k = i+1
    #         break
    # if select_k == 1: #if scores can only be clustered into 1 cluster then no attack was detected
    #     print('No attack detected!')
    #     return 0
    # else: #if scores can be clustered into more than 1 cluster then attack was detected
    #     print('Attack Detected!')
    #     return 1
...
def main(args):
    ...
    with ctx:
        ...
        for e in range(epochs):
            ...
            if e > 50:
                #calculate Hessian vector product using global model differences and global mode-update differences
            ...
            #aggregate global model based on aggregation rule and return the global model gradient and the list of  malicious scores
            ...
            if malicious_score.shape[0] >= 11: #Window size is 10 so it assess the malicious scores when there are 11 or more records of malicious scores
              k = detection1(np.sum(malicious_score[-10:], axis=0), args.nbyz)
              if k: #checks if an attack is detected
                ...
                detectHonest(np.sum(malicious_score[-10:], axis=0), args.nbyz, k)
                #detectMalicious(np.sum(malicious_score[-10:], axis=0), args.nbyz, k)
                test_accuracy = evaluate_accuracy(test_data, net)
                print("Epoch %02d. Test_acc %0.4f" % (e, test_accuracy))
                break
                #if detection1(np.sum(malicious_score[-10:], axis=0), args.nbyz): #checks if an attack is detected
                    #print('Stop at iteration:', e) #stops the process when malicious client is detected
                    #detection(np.sum(malicious_score[-10:], axis=0), args.nbyz) #
                    #break
